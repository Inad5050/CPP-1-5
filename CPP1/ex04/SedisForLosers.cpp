...
std::string	SedisForLosers(std::string fileName1, std::string str1, std::string str2)
{
	...
	while (file1_content[i])
	{
		...
		if (i == (int)std::string::npos) //when .find doesnt find its string it sets i to a uge value, usually -1 converted to unsigned 
			break;
